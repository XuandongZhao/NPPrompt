...
class MicrosoftProcessor(DataProcessor):
    ...
    def __init__(self):
        super().__init__()
        self.labels = ["covid", "fda", "web"]

    def get_examples(self, data_dir, split):
        df = pd.read_csv(data_dir)
        # df = pd.read_csv('fda_covid_web_final.csv')
        label = df['Label'].tolist()
        query = df['Query'].tolist()
        examples = []
        # label_file = open(os.path.join(data_dir, "{}_labels.txt".format(split)), 'r')
        labels = {"Covid": 0, "FDA": 1, "WEB": 2}
        for i in range(len(label)):
            example = InputExample(guid=str(i), text_a=query[i], label=int(labels[label[i]]))
            examples.append(example)
        return examples
